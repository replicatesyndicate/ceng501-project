...
keeping the replay buffer intact.
 ...
 - We do not have multiple agents or a softmax composition (that is RDE).
 - This is the intermediate step: "DQN + Reset," to see if performance collapses.

Usage:
1) Train:
   python reset_dqn.py
   (will write "q_network.pth" after training)

2) Evaluate:
   python reset_dqn.py --evaluate
   (loads "q_network.pth" and runs e.g. 5 episodes, prints average reward)
"""
...
RESET_FREQUENCY    = 40_000   # e.g., for Freeway from the table
...
class ResetDQNAgent:
    ...
    DQN agent that:
      ...
      - No ensembles or softmax composition
      - Otherwise identical to the simple DQN approach
    """
